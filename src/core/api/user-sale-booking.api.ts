...
const baseUrl = '/v1/user-sale-booking';
export interface UserSaleBookingIV1Get extends IPagingDto {}

export interface UserSaleBookingIV1Update extends Pick<UserSaleBooking, 'name' | 'description' | 'note' | 'price' | 'status' | 'imageUrls'> {
...
export const userSaleBookingApi = {
    ...
    v1Put: async (id: string, dto: UserSaleBookingIV1Update) => {
        const url = `${baseUrl}/${id}`;
